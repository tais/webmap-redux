...
// Resolve each sector's .dat by Redux VFS priority: ordered loose MAPS_DIRS (Data-DMK/Maps overrides
// Data-1.13/Maps overrides base Data/Maps), then the base Maps.slf for any sector nobody ships loose.
// First dir wins. This yields Redux's full 255-map set plus base fallbacks.
function createMapSource() {
  const m = new Map(); // UPPERCASE code -> { path, tag } (highest-priority dir wins)
  const tags = ['Data-DMK', 'Data-1.13', 'Data'];
  cfg.MAPS_DIRS.forEach((dir, i) => {
    let files; try { files = fs.readdirSync(dir); } catch (e) { return; }
    const tag = tags[i] || ('dir' + i);
    for (const f of files) { if (!/\.dat$/i.test(f)) continue; const code = f.replace(/\.dat$/i, '').toUpperCase(); if (!m.has(code)) m.set(code, { path: path.join(dir, f), tag }); }
  });
  let slf = null;
  const codes = new Set(m.keys());
  if (cfg.BASE_MAPS_SLF && fs.existsSync(cfg.BASE_MAPS_SLF)) {
    slf = openSlf(cfg.BASE_MAPS_SLF);
    for (const n of slf.names()) { const b = path.basename(n); if (/\.dat$/i.test(b)) codes.add(b.replace(/\.dat$/i, '').toUpperCase()); }
  }
  function resolve(code) {
    const e = m.get(code.toUpperCase());
    if (e) return { buf: fs.readFileSync(e.path), src: e.tag };
    if (slf) { const buf = slf.get(code + '.dat'); if (buf) return { buf, src: 'Maps.slf' }; }
    return null;
  }
...
function buildAll(detailScale, overviewScale, limit, webp, webpQ) {
  ...
  const resolver = createTilesetResolver(cfg.JA2SET_XML, cfg.TILESET_DIRS, cfg.BASE_TILESETS_SLF);
